...
import { Drawer, Input, InputNumber, Button, Space, Typography, Alert, message } from "antd";
 ...
 */
export default function GaslessTransfer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Drawer
      ...
      width={461}
      open={open}
      onClose={() => { onClose(); setTxHash(""); }}
      destroyOnHidden
    >
      <Space direction="vertical" size={14} style={{ width: "100%" }}>
        {/* 免 gas 绿色 Banner（醒目提示平台代付） */}
        <Alert
          type="success"
          showIcon
          message="本次转账 Gas 由平台代付"
          description="您无需持有 ETH 即可完成合规转账（EIP-2771 元交易）"
        ...
        {!isConnected ? (
          ...
          <>
            <Paragraph type="secondary" style={{ fontSize: 13, marginBottom: 0 }}>
              接收方必须已完成 KYC 认证并加入白名单；转账将按平台费率扣除手续费。
            </Paragraph>
            <Input
            ...
            <InputNumber
              ...
              addonAfter="RVGOLD"
            />
            <Button type="primary" block size="large" loading={sending} onClick={handleTransfer} disabled={!cfg}>
              {sending ? "签名并上链中..." : "确认免 Gas 转账"}
            </Button>
            {txHash && (
              <Alert
                type="success"
                showIcon
                message="转账成功"
                description={<Text code style={{ fontSize: 12, wordBreak: "break-all" }}>{txHash}</Text>}
              />
            )}
